...
int main(int argc, char *argv[]){
	...
	N_monomer = 55 ;
	snprintf(datapath, sizeof datapath, "./data.ABseq.%d", N_monomer);
	N_dimension = N_monomer-2 ;

	N_population = 30 ;

	N_iteration = 2*10000000 ;
	...
	grid_low = -19.0 ;
	grid_high = 0.0 ;
	grid_size = floor((grid_high-grid_low)/0.1) ;
	theta_norm_const = 1.0 ;
	...
	for (i = 1; i < argc; i++) {
		...
		else if (strcmp("-Nmonomer", argv[i]) == 0)
		{
			N_monomer = atoi(argv[++i]) ;
			N_dimension = N_monomer-2 ;
		}
		...
		else if (strcmp("-SMO0", argv[i]) == 0)			/*..SCALING*/
			scl_mh0 = atof(argv[++i]) ;
	...
	/*GET DATA -----------------------------------------------------------*/
	printf("\n GET DATA\n") ;
	get_data(datapath, N_monomer, &N_dimension) ;
	...
	for (pop=1 ; pop<=N_population; pop++) {
		for (i=1; i<=N_dimension; i++) {
			...
			do { un = normalrng() ; } while(un<-3.5 || un>3.5) ;
			x[pop][i] = un ;
		}
	...
	for( iter=1 ; iter<=N_iteration ; iter++ ) {
		...
		case 0 :
			...
			n_mh0++ ;
			/*scaling adaptation*/
			/*scl_mh0+=pow(5.0/n_mh0,0.6)*(accpr-0.23) ;*/
			/*scl_mh0*=exp(pow(5.0/n_mh0,0.6)*(accpr-0.2)) ;*/
			break ;
		case 1 :
			/*sample*/
			sumv = 0.0 ;
			...
			n_mh1++ ;
			/*scaling adaptation*/
			if (scl_mh1 > 10) scl_mh1 = 1.4142 ;
			scl_mh1*=exp(pow(5.0/n_mh1,0.6)*(accpr-0.2)) ;
			break ;
		case 2 :
			/*sample*/
			sumv = 0.0 ;
			...
			n_mh2++ ;
			/*scaling adaptation*/
			/*if (scl_mh2 > 10) scl_mh2 = 1.0 ;
			scl_mh2*=exp(pow(5.0/n_mh2,0.6)*(accpr-0.2)) ;*/
			break ;
		case 3 :
			/*sample*/
			sumv = 0.0 ;
			...
			n_mh3++ ;
			/*scaling adaptation*/
			if (scl_mh3 > 10.0) scl_mh3 = 1.0 ;
				scl_mh3*=exp(pow(5.0/n_mh3,0.6)*(accpr-0.5)) ;
			break ;
		case 4 :
			/*sample*/
		...
		if (ins_fz_best_trace != NULL && N_sample>0)
		if (N_sample >0)
		if ( iter>0 && iter%N_thinning==0 ) {
			fprintf(ins_fz_best_trace,"%d %15.12f \n", iter, fz_best) ;
